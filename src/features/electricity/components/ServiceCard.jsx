...
export function ServiceCard({ id, service, refreshing, isFlashing, onRefresh, onEdit, onShowQR, onAbout, onDelete, onTogglePin, onPay, useAccordion, selected, selecting, onToggleSelect, onCalculateBill }) {
  ...
  return (
    ...
    >
          ...
          <div className="scard__menu-wrap">
            ...
            {menuOpen && (
              <div className="popover" onMouseDown={e => e.stopPropagation()} style={{ zIndex: 100 }}>
                ...
                <button onMouseDown={(e) => { e.stopPropagation(); setMenuOpen(false); onShowQR?.(service); }}>
                  <FiClock size={13} /> Update Bill Time
                </button>
      ...
      <div className="scard__hero-main" onClick={useAccordion ? () => setIsExpanded(!isExpanded) : undefined}>
        <div className="scard__hero-content">
          ...
          <div className="scard__hero-meta">
            ...
            {service.lastError && (
              <span className="text-danger" style={{ display: 'flex', alignItems: 'center', gap: '4px', marginLeft: '8px' }}>
                <FiAlertTriangle size={12} /> Sync Error
              </span>
            )}
          </div>
        </div>
      ...
      <div className="scard__quick-metrics" onClick={useAccordion ? () => setIsExpanded(!isExpanded) : undefined} style={{ cursor: useAccordion ? 'pointer' : 'default' }}>
        ...
        {insights && (
          <div className="qm-item">
            <span className="qm-label">{t('avg_mo')}</span>
            <span className="qm-val">{formatInr(insights.avgAmount)}</span>
          </div>
        )}
      </div>
      ...
      <div className="scard__action-bar" onClick={e => e.stopPropagation()} style={{ position: 'relative', zIndex: 20 }}>
        <div className="scard__action-left">
          <button className={`btn-ghost-sm ${refreshing ? 'spin' : ''}`} onClick={onRefresh} disabled={refreshing}>
      ...
      <div className={`scard__body ${isExpanded ? 'scard__body--expanded' : ''}`}>
        <div className="scard__body-inner" style={{ overflow: 'visible' }}>
          {insights && (
            <Section title="Consumption Insights" defaultOpen={true}>
              <div style={{ padding: '0 10px' }}>
                 <div className="receipt-row">
                    <span className="receipt-row__label">Vs Last Month</span>
                    <TrendBadge value={insights.vsLastMonth?.units} unit="u" percent={insights.vsLastMonth?.unitsPct} />
                 </div>
                 {insights.vsSameMonthLastYear && (
                   <div className="receipt-row">
                      <span className="receipt-row__label">Vs Last Year</span>
                 ...
                 <div className="receipt-row">
                    <span className="receipt-row__label">Last Updated</span>
                    <b className="receipt-row__val">{fromNow(service.lastFetchedAt)}</b>
                 </div>
              </div>
            </Section>
          )}
